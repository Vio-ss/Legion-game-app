...
@Component({
  selector: 'app-savegame',
  templateUrl: './savegame.component.html',
  styleUrls: ['./savegame.component.css'],
...
export class SavegameComponent {
  ...
  ngOnInit() {
    this.service.getsaves().subscribe(data => {
      this.data = data;
    });
  }
  ...
  Loadgame() {
    ...
    this.service.GetAllSaves().subscribe(res => {
  ...
  wishgame(id:any, name:any, category:any, year:any, mode:any, size:any, link:any){
    this.service.wishgame(id, name, category,year, mode, size, link).subscribe({
      next: (val:any)=>{
        this.toastr.success('Bookmarked successfully.');
